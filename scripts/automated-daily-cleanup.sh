...
load_config() {
    if [[ -f "$CONFIG_FILE" ]]; then
        ...
        # Use Python script to parse YAML configuration properly
        if command -v python3 &> /dev/null && [[ -f "$SCRIPT_DIR/parse_config.py" ]]; then
            ENABLED_ENVIRONMENTS=$(python3 "$SCRIPT_DIR/parse_config.py" "$CONFIG_FILE" "enabledEnvironments" 2>/dev/null || echo "dev,sit")
            NOTIFICATION_EMAIL=$(python3 "$SCRIPT_DIR/parse_config.py" "$CONFIG_FILE" "notificationEmail" 2>/dev/null || echo "")
            SLACK_WEBHOOK=$(python3 "$SCRIPT_DIR/parse_config.py" "$CONFIG_FILE" "slackWebhook" 2>/dev/null || echo "")
            DRY_RUN=$(python3 "$SCRIPT_DIR/parse_config.py" "$CONFIG_FILE" "dryRun" 2>/dev/null || echo "false")
        else
            log "WARN" "Python3 or config parser not found. Using default configuration."
        fi
    else
...
# Get resource groups for environment
get_resource_groups() {
...
# Perform cleanup for a specific resource group
